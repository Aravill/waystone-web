...
window.campaignsPage = function() {
    return {
        ...
        renderCampaignHtml(campaign) {
            ...
            const dmHtml = dmUser ? 
                `<a href="${escapeHtml(dmUser.profile_url || `/profile?user_id=${encodeURIComponent(dmUser.id)}`)}" class="user-button">${escapeHtml(dmUser.display_name || dmUser.name || dmUser.email || 'Unknown')}</a>` :
            ...
            if (playerUsers.length === 0 && players.length === 0) {
                ...
                playersHtml = playerUsers.map(p => 
                    `<a href="${escapeHtml(p.profile_url || `/profile?user_id=${encodeURIComponent(p.id)}`)}" class="user-button">${escapeHtml(p.display_name || p.name || p.email || 'Unknown')}</a>`
                ).join('');
            } else {
                playersHtml = players.map(p => 
                    `<span class="campaign-nameplate">${escapeHtml(p)}</span>`
                ).join('');
            }
            
            return `
                <article class="campaign-item" id="campaign-${escapeHtml(campaign?.id ?? 'unknown')}">
                    ...
                    <section class="campaign-row campaign-row-middle">
                        <div class="campaign-column campaign-column-left">
                            <div class="campaign-field">
                                <h4 class="campaign-label">Status</h4>
                                <p class="campaign-value">${escapeHtml(status)}</p>
                            </div>
                            <div class="campaign-field">
