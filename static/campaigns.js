...
window.campaignsPage = function () {
    return {
        ...
        async showResponseBreakdown(session) {
            ...
            try {
                const response = await fetch(`/api/campaigns/${this.currentCampaignId}/sessions/${targetSession.id}/responses`);
                if (!response.ok) {
                    throw new Error('Failed to load responses');
                }
                const payload = await response.json();
                this.sessionResponses = Array.isArray(payload.all) ? payload.all : [];
                this.pendingResponses = Array.isArray(payload.pending) ? payload.pending : [];
